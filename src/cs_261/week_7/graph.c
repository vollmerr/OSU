...
{
	clearVisited(graph);
	struct Deque* visited = dequeNew();
	dequePushBack(visited, source);

	while (!dequeIsEmpty(visited)) {
		Vertex* vertex = dequeBack(visited);
		if (vertex == destination) {
			dequeDelete(visited);
			return 1;
		}
		vertex->isVisited = 1;
		dequePopBack(visited);
		for (int i = 0; i < vertex->numNeighbors; ++i) {
			if (!vertex->neighbors[i]->isVisited) {
				dequePushBack(visited, vertex->neighbors[i]);
			}
		}
	}
	dequeDelete(visited);
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	while (!dequeIsEmpty(visited)) {
		Vertex* vertex = dequeFront(visited);
		if (vertex == destination) {
			dequeDelete(visited);
			return 1;
		}
		vertex->isVisited = 1;
		dequePopFront(visited);
		for (int i = 0; i < vertex->numNeighbors; ++i) {
			if (!vertex->neighbors[i]->isVisited) {
				dequePushBack(visited, vertex->neighbors[i]);
			}
